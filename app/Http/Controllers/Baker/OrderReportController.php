    ...
    public function index()
    {
        $orders = DB::table('orders')
            ...
            ->select(
                ...
                'orders.subtotal',
                'orders.tax',
                'orders.total_amount',          // ← was: orders.total
                'orders.seller_id',
                'orders.baker_id',
                'orders.created_at',
    ...
    public function show($id)
{
    ...
    $items = DB::table('order_items')
        ->leftJoin('products', 'order_items.product_id', '=', 'products.id') // Changed to leftJoin
        ->select(
            'order_items.id',
            'order_items.quantity',
            'order_items.unit_price',
            DB::raw("IFNULL(products.name, 'Unknown Product') as product_name"), // Handle deleted products
            'products.category',
